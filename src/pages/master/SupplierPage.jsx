...
/* ─── Item Row (Items supplied by this vendor) ───────────────────────────── */
...
export default function SupplierCreationPage() {
  ...
  const [form, setForm] = useState({
    // Basic
    supplierCode: '', supplierName: '', printName: '', supplierGroup: '',
    supplierType: '', territory: '', industry: '', status: 'Active',
    // Address
    ...
    msmeNo: '', msmeType: '', tdsApplicable: false,
    // Financial
    currency: 'INR - Indian Rupee', paymentTerms: '', creditLimit: '',
    creditDays: '', openingBalance: '', openingBalanceType: 'Cr',
    ledgerGroup: '', discount: '',
    // Purchase
    purchaseOrderType: '', minOrderQty: '', minOrderValue: '',
    deliveryTerms: '', transportMode: '', transporter: '',
    qualityRequired: false, inspectionRequired: false,
    // Flags
    active: true,
  })
      ...
      <div style={{ margin: '0 auto' }}>
        ...
        <Section title="Basic Information" icon={Truck}>
          <Grid cols={3}>
            ...
            <Select label="Supplier Group" value={form.supplierGroup} onChange={e => set('supplierGroup', e.target.value)}
              options={['Raw Material','Component','Packing','Service','Subcontractor','Tooling','Capital Goods','Others']} />
            <Select label="Supplier Type" value={form.supplierType} onChange={e => set('supplierType', e.target.value)}
              options={['Manufacturer','Trader','Importer','Service Provider','Subcontractor','OEM']} />
            ...
            <Select label="Status" value={form.status} onChange={e => set('status', e.target.value)}
              options={['Active','Inactive','Blocked','Under Evaluation','Approved','Blacklisted']} />
          </Grid>
        ...
        <Section title="Contact Information" icon={Phone}>
          ...
          <p style={{ fontSize: '11px', fontWeight: '700', color: '#8F6593', textTransform: 'uppercase', letterSpacing: '0.08em', margin: '16px 0 10px' }}>Contact Persons</p>
          {contacts.map((c, i) => (
        ...
        <Section title="Purchase Terms" icon={Package}>
          <Grid cols={3}>
            ...
            <Input label="Min Order Value (₹)" {...bind('minOrderValue')} placeholder="0.00" type="number" />
            ...
            <Select label="Delivery Terms" value={form.deliveryTerms} onChange={e => set('deliveryTerms', e.target.value)}
              options={['Ex-Works','FOB','CIF','Door Delivery','FOR Destination']} />
